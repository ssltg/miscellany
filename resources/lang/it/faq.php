...
return [
    'attribute-templates'   => [
        'answer'    => "Il miglior modo in cui possiamo spiegarti i Template di Attributi è con un esempio. Immaginiamo per esempio che il tuo mondo abbia molti Luoghi e su molti di questi luoghi vuoi ricordarti di creare degli attributi personalizzati per \"Popolazione\", \"Clima\", \"Livello di Criminalità\".
...
Supportarci su Patreon però ti permette di incrementare il limite di dimensione dei file che puoi caricare, aggiunge il tuo nome al wall of fame dei Patreon, avere delle icone di default più curate, votare per prioritizzare cosa verrà sviluppato ed altro ancora!",
    ...
    'help'                  => [
        'answer'    => 'Prima di tutto grazie per volerci aiutare! Siamo sempre interessati in persone che possono aiutarci con le traduzioni, nel testare le nuove funzionalità o che possano aiutare i nuovi utenti. Adoriamo anche quando le persone promuovono ' . config('app.name') . ' per raggiungere nuove utenze in posti a cui non avevamo pensato. La miglior cosa che puoi fare è unirti a noi su Discord dove un canale è dedicato all\'aiuto degli utenti. Adoriamo anche i nostri patrons su Patreon se vuoi supportarci e ottenere l\'accesso a qualche vantaggio!',
        'question'  => 'Voglio aiutare! Cosa posso fare?',
    ],
    'map'                   => [
        'answer'    => "Ogno luogo può contenere una mappa (png, jpg o svg) con all'interno dei \"punti della mappa\" che possono essere posizionati controllandone la dimensione, la forma, l'icona ed il colore impostandoli come collegamenti ad altre entità o semplici etichette.

Per favore considerate che le mappe prodotte da tool popolari come Azgaar e Medieval Fantasy Town Generator comprimono iol file generato rendendoli incompatibili con " . config('app.name') . ". Un fix è quello di aprire il file con Inkscape o Photoshop e salvarlo nuovamente prima di caricarlo su " . config('app.name') . ".",
        'question'  => 'Posso caricare delle mappe su ' . config('app.name') . '?',
    ],
    'mobile'                => [
        'answer'    => 'Attualmente non vi è nessuna app mobile per ' . config('app.name') . ' ma la maggior parte delle funzionalità funzionano anche su di un dispositivo mobile. Una limitazione è lo strumento di menzione che non funziona nell\'editor testuale. Se il supporto su Patreon lo permetterà, spero un giorno di riuscire a pagare qualcuno per fare l\'app mobile ma non prevedo che succederà a breve.',
    ...
    'permissions'           => [
        'answer'    => 'Assolutamente, questo è il perché noi abbiamo creato ' . config('app.name') . '! Potrai invitare tutti i tuoi giocatori nella tua campagna ed assegnargli dei ruoli e dei permessi. Abbiamo costruito il tutto per essere estremamente flessibile (tu potrai utilizzare sia sia una configurazione opt-in che una opt-out) per coprire il maggiorn numero possibile di bisogni e situazioni.',
        'question'  => 'Io voglio utilizzare ' . config('app.name') . ' per costruire il mondo del mio RPG, ma voglio che i miei giocatori abbiano accesso ad alcune delle entità e possano modificare i loro personaggi. È possibile?',
    ],
    'plans'                 => [
        'answer'    => "I piani a lungo termine per " . config('app.name') . " sono di creare un versatile strumento di gestione delle campagne e di creazione di mondi che sia indipendente dal sistema utilizzato con contenuti specifici per il systema gestiti dalla comunità nella forma di \"Template della Comunità\". Un traguardo successivo sarà quello di realezzare uno strumento che possa integrarsi con altre piattaforma come Virtual Table Top per collegarle con i mondi su " . config('app.name') . ".

Per quanto riguarda la seconda parte la maggior parte dei progetti obbistici finiscono per mancanza di fondi e con il creatore che li abbandona. Il Patreon è stato pensato per fare in modo che possa ridurre le mie ore lavorative per dedicare più tempo a ' . config('app.name') . ' senza sacrificare la sicurezza finanziaria della mia famiglia e per coprire i costi del server. Il progetto è anche open source e potrà essere gestito dalla comunità se quelcosa dovesse mai succedermi. I dati di ogni campagna possono essere esportati dall'amministratore della stessa una volta al giorno in caso fossi preoccupato per la possibilità di perdere tutti i tuoi contenuti.",
        'question'  => 'Quali sono i piani a lungo termine? Cosa succederebbe se Ilestis si stufasse di lavorare su ' . config('app.name') . '?',
    ],
    'show'                  => [
