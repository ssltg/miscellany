...
Cependant, nous supporter sur Patreon te permet d'augmenter la taille maximal des images à uploader, d'apparaitre dans le \"Hall of Fame Patreon\", d'avoir des icons plus sympas, de voter sur les tickets a prioritiser, et bien plus!",
    ...
    'map'                   => [
        'answer'    => "Chaque lieu peut contenir une carte (png, jpg ou svg) qui elle-même contient des \"points\" qui sont placés sur la carte avec une taille, forme, icône et couleur, et ces points peuvent être lié à des entités ou un simple text.

Attention cependant, les images générées par Azgaar et watabou.io sont comprimés de tel à ce qu'ils sont incompatibles avec " . config('app.name') . ". Une solution est d'ouvrir les images dans Inkscape ou Photoshop et simplement réenregister les images SVG avant de les uploadé à ' . config('app.name') . '.
"
,
        'question'  => 'Puis-je uploadé des cartes sur ' . config('app.name') . '?',
    ],
    'mobile'                => [
        'answer'    => 'Il n\'y a actuellement pas d\'application mobile pour ' . config('app.name') . ', mais la majorité des fonctions fonctionnent sur un browser mobile. Une limitation est l\'outil de mention ne fonctionnant pas dans l\'éditeur de text. Si le support Patreon le permet, j\'espère un jour pouvoir payer quelqu\'un pour construire et maintenir une application mobile.',
    ...
    'permissions'           => [
        'answer'    => 'Absolument, c\'est ce pourquoi nous avons fait ' . config('app.name') . '! Vous pouvez inviter tous les joueurs à votre campagne, et leurs donner des rôles et des permissions. Nous avons réalisé un système extrêmement flexible (vous pouvez à la fois utiliser des options de configuration de permission et de restriction) pour couvrir toutes les situations possibles dont vous pourriez avoir besoin.',
        'question'  => 'Je veux utiliser ' . config('app.name') . ' pour concevoir mon propre univers JDR, mais je souhaiterais que mes joueurs puissent avoir accès à certains éléments et modifier leurs personnages. Est-ce possible ?',
    ],
    'plans'                 => [
        'answer'    => "
Les plans long terms pour " . config('app.name') . " sont de construire une app flexible pour les autheurs et maîtres de jeu qui est indépendant d'un système de jeu, avec du contenu communautaire pour tout ce qui est spéficique à un système de jeu. Un objectif plus long term est de construire des méchanisme entre " . config('app.name') . " et des applications de jeu virtual pour que ceux-ci accèdent aux données de " . config('app.name') . ".

Concernant le second point, la plupart des projets hobby finissent en burnout pour le créateur. Le Patreon est mise en place avec l'objectif de me permettre de réduire mon temps de travail, pour que je puisse dédier plus de temps à " . config('app.name') . " sans sacrifier la stabilité financière de ma famille. Ce project est aussi open source et peut être continué et amélioré par la communauté si quelque chose devait m'arriver. Le contenu de chaque campagne peut être exporté une fois par jour pas un admin d'une campagne en cas de doute sur la sécurité des données à long term.",
        'question'  => 'Quelles sont les objectifs long terms? Que ce passe-t-il si Ilestis ne souhaite plus travailler sur ' . config('app.name') . '?',
    ],
    'show'                  => [
